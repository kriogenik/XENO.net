...
3. **Алерты без спама:** open → remind(6ч) → recover; стабильные fingerprints.  
4. **Наблюдаемость файлами + Telegram**, не обязательный Prometheus. Журнал: `/var/log/xeno/events.jsonl` — см. [observability.md](observability.md).  
5. **Чиним корень**, не mute. Watchdog / canary / unit Restart=always — да; «выключить алерт» — нет.
