...
| «Оба мёртвы» NOW, а на NL `:2053` ESTAB с IP юзера + accepts минуту назад | Туннель жив; Happ ping/UI красный или смотрят не тот профиль | Сверить ESTAB/`ss` + access email; sub URI↔Reality; E2E с VPS по UUID | VPN выкл → refresh той же `https://` → вручную **🇳🇱 Direct** (сайт), затем **🇷🇺 RU** 60с; не ротировать UUID |
...
| «У всех RU мёртв», Direct OK; IP Timeweb; spamlist чист | Было: гипотеза ТСПУ/гео-mismatch Google SNI → retarget timeweb (Aug 7). **Aug 8 restore:** снова согласованный Google на BRIDGE (пик Aug 3–4) для A/B | Sub URI `sni` == live `serverNames`; не смешивать Google URI с timeweb Reality | Refresh `:2080`; вручную **🇷🇺 RU** 60с; смотреть accepts с PC |
